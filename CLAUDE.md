...
| 项目 | 值 |
|------|-----|
| GitHub 仓库 | `https://github.com/erichecan/IErepair.git` |
| GCP Project ID | `supply-491510` |
| GCP Project Number | `549968261036` |
| Cloud Run 服务名 | `ierepair` |
| 数据库 | Neon PostgreSQL（Frankfurt, eu-central-1） |
| 部署方式 | Cloud Run 多区域 + Global Load Balancer |

**线上地址（各区域直接访问）：**
- 欧洲（推荐，距数据库最近）：https://ierepair-549968261036.europe-west1.run.app
- 美洲：https://ierepair-549968261036.us-central1.run.app
- 亚洲：https://ierepair-549968261036.asia-east1.run.app

**全球负载均衡器（HTTP，自动路由到最近区域）：**
- IP: `34.96.66.15`（http://34.96.66.15）
- 备注：如需 HTTPS 全球路由，需配置自定义域名 + Google Managed SSL Certificate

**部署命令（每次发布执行，会同时更新三个区域）：**
```bash
# 1. 本地构建
npm run build
...
**所有 UI 开发必须遵循 `DESIGN.md` 中定义的 Cal.com 设计风格。**

核心原则：
- 配色：纯灰度系统，主文字 `#242424`，次要文字 `#898989`，背景 `#ffffff`
- 字体：标题用 Cal Sans（weight 600），正文用 Inter，严格不混用
- 阴影代替边框：卡片使用多层阴影 `rgba(34,42,53,0.08) 0px 0px 0px 1px` 作为边框
- 间距慷慨：section 间距 80px+，营造高级感
- 禁止使用装饰性插图或非灰度品牌色

每次写 UI 组件前先读 `DESIGN.md`。
