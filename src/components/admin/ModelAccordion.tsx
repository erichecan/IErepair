...
export default function ModelAccordion({
  ...
  async function handleAddModel() {
    ...
    if (res.ok) {
      ...
      setAddingModel(false); setNewModelName(""); setNewModelYear("");
    } else {
  ...
  function openAddService(modelId: number) {
    setExpanded(prev => { const s = new Set(prev); s.add(modelId); return s; });
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
        {addingModel ? (
          <div style={{
            ...
            padding: "10px 14px", background: "#f4faf7", borderRadius: 12,
            border: "1px solid #c3e8d5", width: "100%",
          }}>
            <input
              autoFocus
              value={newModelName}
            ...
            <button onClick={handleAddModel} disabled={addModelLoading} style={{ ...btnPrimary, opacity: addModelLoading ? 0.6 : 1 }}>
              {addModelLoading ? "…" : "创建"}
            </button>
            <button onClick={() => { setAddingModel(false); setNewModelName(""); setNewModelYear(""); setAddModelError(""); }} style={btnCancel}>
              取消
            </button>
          </div>
        ) : (
          ...
          {models.map(model => {
            ...
            return (
              <div key={model.id} style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e5ea", overflow: "hidden" }}>
                  ...
                  <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
                    <span style={{ fontSize: 15, fontWeight: 600, color: "#1d1d1f" }}>
                      <InlineEdit
                        ...
                        onCommit={commitEdit} onCancel={cancelEdit} inputType="text" inputWidth={180}
                      />
                    </span>
                    <span style={{ fontSize: 12, color: "#aeaeb2" }}>
                      <InlineEdit
                        ...
                        onChange={v => setEditCell(prev => prev ? { ...prev, value: v } : null)}
                        onCommit={commitEdit} onCancel={cancelEdit} inputType="number" inputWidth={68}
                      />
                    </span>
                  </div>
                ...
                {isOpen && addingService === model.id && (
                  <div style={{ padding: "10px 20px", borderBottom: "1px solid #ebebeb", background: "#f4faf7" }}>
