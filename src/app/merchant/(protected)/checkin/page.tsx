...
export default function MerchantCheckinPage() {
  ...
  async function startScan() {
    ...
    try {
      ...
      setState("error");
      setErrorMsg("无法访问摄像头，请检查浏览器权限或使用手动输入。");
    }
  }
  ...
  async function doCheckin(orderNumber: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setErrorMsg(data.error || "Check-in 失败");
        setState("error");
      }
    } catch {
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: "#1d1d1f", margin: "0 0 4px" }}>扫码 Check-in</h1>
        <p style={{ fontSize: 14, color: "#6e6e73" }}>扫描客户手机上的预约二维码，将预约状态更新为「维修中」。</p>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
        <div style={{ position: "relative", width: 320, height: 320, borderRadius: 16, overflow: "hidden", background: "#1d1d1f", border: "2px solid #e5e5ea" }}>
          ...
          {state !== "scanning" && (
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "#fff", gap: 12 }}>
              ...
              <div style={{ fontSize: 14, opacity: 0.7 }}>点击下方按钮开始扫描</div>
            </div>
          )}
          {state === "scanning" && (
        ...
        {state === "success" && result && (
          <div style={{ background: "#e8f7f0", border: "1px solid #146345", borderRadius: 14, padding: "20px 28px", width: "100%", maxWidth: 480 }}>
            <div style={{ fontSize: 22, marginBottom: 8 }}>✅ Check-in 成功</div>
            <div style={{ display: "grid", gap: 6, fontSize: 14 }}>
              <div><span style={{ color: "#888", minWidth: 80, display: "inline-block" }}>订单号</span><strong style={{ fontFamily: "monospace" }}>{result.orderNumber}</strong></div>
              <div><span style={{ color: "#888", minWidth: 80, display: "inline-block" }}>客户</span><strong>{result.userName}</strong></div>
              <div><span style={{ color: "#888", minWidth: 80, display: "inline-block" }}>设备</span><span>{result.device}</span></div>
              <div><span style={{ color: "#888", minWidth: 80, display: "inline-block" }}>维修项</span><span>{result.repairType}</span></div>
            </div>
            <div style={{ marginTop: 12, fontSize: 13, color: "#146345", fontWeight: 600 }}>预约状态已更新为「维修中」</div>
          </div>
        )}
        ...
        <div style={{ width: "100%", maxWidth: 480, marginTop: 8 }}>
          <div style={{ fontSize: 13, color: "#888", marginBottom: 8, textAlign: "center" }}>或手动输入订单号</div>
