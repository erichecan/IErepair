...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const item = await prisma.merchantService.findUnique({ where: { id: parseInt(id) } });
    ...
    const updated = await prisma.merchantService.update({ where: { id: parseInt(id) }, data });
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.merchantService.delete({ where: { id: parseInt(id) } });
